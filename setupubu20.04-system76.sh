...
if ! hash wpsoffice 2>/dev/null;then
        echo -e $Red "Not Installed [✗]"
	${White} wget https://wdl1.pcfg.cache.wpscdn.com/wpsdl/wpsoffice/download/linux/10161/wps-office_11.1.0.10161.XA_amd64.deb
	sudo dpkg -i wps-office_11.1.0.10161.XA_amd64.deb
	rm wps-office_11.1.0.10161.XA_amd64.deb
else
	echo -e $Green "WPSOffice installed [✓]"
   ${White}
   sleep 2
fi
${White}
...
if [ -f "/etc/apt/sources.list.d/lutris-team-ubuntu-lutris-focal.list" ] 2>/dev/null;then
	...
	echo -e $Green "Lutris-Repo installed [✓]"
   	...
   	echo "Lutris Repo already added"
fi
...
if ! hash lutris 2>/dev/null;then
	...
	echo -e $Green "Lutris installed [✓]"
   	...
   	echo "Lutris does already existing"
fi
clear
sleep 2
